...
r_base = np.load("results/r2_map_mean_base_iro_sar.npy")

# Print mean R^2 values for diagnostics
print(f"Mean R² (Audio+Base): {r_audio.mean():.4f}")
print(f"Mean R² (Text+Base): {r_text.mean():.4f}")
print(f"Mean R² (Text+Audio+Base): {r_text_audio.mean():.4f}")
print(f"Mean R² (Base): {r_base.mean():.4f}")
...
r_base_nifti = nib.Nifti1Image(r_base, affine)
...
r_base = image.smooth_img(r_base_nifti, fwhm=fwhm).get_fdata() * brain_mask
...
print(f"Zero voxels in r_base (within brain): {np.sum(r_base[brain_mask] == 0) / np.sum(brain_mask):.2%}")

# Step 1: Get the original shape
X, Y, Z = r_audio.shape

# Step 2: Compute observed improvement
delta_r_obs_3d = r_text_audio - (r_audio + r_text - r_base)
...
r_base_flat = r_base.reshape(-1)[valid_voxels]
...
for i in range(num_permutations):
    ...
    r_base_perm = r_base_flat[permuted_indices]
    delta_r_perm[:, i] = r_text_audio_perm - (r_audio_perm + r_text_perm - r_base_perm)
...
initial_mask_3d = p_values_3d < 0.05
...
plotting.plot_glass_brain(
    significance_nifti,
    threshold=0,
    title='Significant Delta R² (Text+Audio vs Text+Audio+Base)\nFWER-corrected p < 0.05',
    colorbar=True,
    plot_abs=False,
    display_mode='ortho',
    axes=ax
...
plotting.plot_glass_brain(
    ...
    threshold=0,
    title='Significant Positive Delta R² (Text+Audio > Text+Audio+Base)\nFWER-corrected p < 0.05',
    colorbar=True,
    plot_abs=False,
    display_mode='ortho',
    axes=ax
...
plt.xlabel('Delta R² (Text+Audio - (Text + Audio - Base))')
...
plotting.plot_stat_map(
    delta_r_nifti,
    bg_img=mni_template,
    title='Unthresholded Delta R² (Text+Audio - (Text + Audio - Base))',
    colorbar=True,
    cmap='RdBu_r',
    display_mode='ortho',
    cut_coords=(0, 0, 0)
